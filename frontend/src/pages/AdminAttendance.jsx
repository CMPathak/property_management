...
const parseTime = (timeStr) => {
  if (!timeStr || timeStr === "ΓÇö" || timeStr === "-") return null;
...
export default function AdminAttendance() {
  ...
  const isFullAccess = user?.role === "SUPER_ADMIN" || user?.role === "OWNER" || user?.role === "MANAGER" || (user?.role === "STAFF" && user?.designation === "Property Manager");
  ...
  const [dateFilter, setDateFilter] = useState(""); // Defaulting to empty to show all dates or specific date
  ...
  useEffect(() => {
    ...
    setDateFilter(`${now.getFullYear()}-${mm}-${dd}`);
  }, []);

  const getMergedRecords = () => {
    const todayStr = new Date().toISOString().split("T")[0];
    const shouldAddVirtual = dateFilter === "" || dateFilter === todayStr;
    
    let merged = records.filter(r => r.role !== "TENANT");
      ...
      targetUsers.forEach((staff) => {
        ...
        if (!hasTodayRecord) {
          merged.push({
            ...
            role: staff.role,
            employee_id: staff.employee_id || "ΓÇö",
            designation: staff.designation || "ΓÇö",
            department: staff.department || "ΓÇö",
            shift_timing: staff.shift_timing || "ΓÇö",
            date: todayStr,
            check_in: "ΓÇö",
            check_out: "ΓÇö",
            status: "ABSENT"
  ...
  const filteredRecords = userRecords.filter((r) => {
    if (dateFilter && dateFilter !== "ALL" && r.date !== dateFilter) return false;
  ...
  return (
    <Box sx={{ flexGrow: 1, pb: 4 }} className="fade-in">
      ...
      {/* Filter Row */}
      <Box sx={{ 
        display: "flex", flexWrap: "nowrap", alignItems: "center", gap: 1.5, mb: 3, p: 1.5, px: 2,
        bgcolor: "#fff",
        borderRadius: "12px",
      ...
      {/* Table Area */}
      <TableContainer component={Paper} sx={{ borderRadius: "16px", boxShadow: "0 2px 10px rgba(0,0,0,0.03)", border: "1px solid #E2E8F0" }}>
        <Table sx={{ minWidth: 1000, "& .MuiTableCell-root": { whiteSpace: "nowrap", border: "1px solid #E2E8F0 !important" } }}>
              ...
              paginatedRecords.map((r, idx) => {
                ...
                return (
                  <TableRow key={idx} hover sx={{ bgcolor: "#FFFFFF" }}>
                    ...
                    <TableCell sx={{ color: r.check_in && r.check_in !== "ΓÇö" ? "#16A34A" : "#94A3B8", fontWeight: 700, fontSize: "0.85rem", borderBottom: "1px solid #F8FAFC" }}>
                      {r.check_in || "ΓÇö"}
                    </TableCell>
                    <TableCell sx={{ color: r.check_out && r.check_out !== "ΓÇö" ? "#DC2626" : "#94A3B8", fontWeight: 700, fontSize: "0.85rem", borderBottom: "1px solid #F8FAFC" }}>
                      {r.check_out || "ΓÇö"}
                    </TableCell>
                    <TableCell sx={{ borderBottom: "1px solid #F8FAFC" }}>
