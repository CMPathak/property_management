...
class CRUDTenantProfile(BaseRepository[TenantProfile, TenantProfileCreate, TenantProfileUpdate]):
    ...
    async def check_in(
        ...
        sets tenant status to ACTIVE, and registers check-in date.
        """
        ...
        tenant.check_in_date = check_in_date
    ...
    async def check_out(
        ...
        sets tenant status to INACTIVE, and registers check-out date.
        """
        tenant = await self.get(db, tenant_id)
        ...
        # Update Tenant profile
        tenant.bed_id = None
        tenant.check_out_date = check_out_date
...
class CRUDAgreement(BaseRepository[Agreement, AgreementCreate, AgreementUpdate]):
    async def get_active_agreement(self, db: AsyncSession, tenant_profile_id: uuid.UUID) -> Agreement | None:
        statement = select(Agreement).where(
            Agreement.tenant_profile_id == tenant_profile_id,
